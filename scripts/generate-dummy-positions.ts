/**
 * Dummy Position Data Generator
 * 
 * Generates realistic test data for the position management system
 * Run: npx ts-node scripts/generate-dummy-positions.ts
 */
...
const EXIT_REASONS = [
  ...
  'Earnings approaching - Reducing risk',
  'Hit time stop - No follow-through',
]
...
const LESSONS = [
  ...
  'Let emotions override plan. Held too long.',
  'Entry was rushed. Should have waited for pullback.',
...
async function generateClosedPosition(ticker: any, index: number) {
  ...
  const position = {
    ...
    catalyst: Math.random() > 0.5 ? randomElement(['Earnings', 'Breakout', 'News', 'Sector Rotation']) : null,
    market_conditions: randomElement(MARKET_CONDITIONS),
    
    // Journal
    entry_thesis: randomElement(THESES),
    exit_plan: `Target: $${target.toFixed(2)} | Stop: $${stopLoss.toFixed(2)} | R:R = 1:${randomDecimal(2, 5)}`,
    ...
    // Technical
    chart_pattern: randomElement(['Bull Flag', 'Triangle', 'H&S', 'Double Bottom', 'Cup & Handle']),
  ...
  // Add some notes
  if (Math.random() > 0.5) {
    await addNote(data.id, randomElement([
      'Watching for continuation',
      'Strong setup - high conviction',
...
async function generateOpenPosition(ticker: any) {
  const direction = 'long'
  ...
  const position = {
    ...
    setup_type: randomElement(SETUP_TYPES),
    time_frame: 'swing',
    catalyst: Math.random() > 0.5 ? randomElement(['Earnings', 'Breakout', 'Momentum']) : null,
    market_conditions: randomElement(MARKET_CONDITIONS),
    
    // Journal
    entry_thesis: randomElement(THESES),
    exit_plan: `Target: $${target.toFixed(2)} | Stop: $${stopLoss.toFixed(2)} | Scale out at T1 (50%), T2 (30%), T3 (20%)`,
  ...
  if (Math.random() > 0.3) {
    await addNote(data.id, randomElement([
      ...
      'Watching for break above resistance',
      'Will add if pulls back to support',
      'Consider taking partial profits soon',
...
async function addActivityHistory(
  positionId: string,
  ticker: string,
  entryDate: Date,
  daysHeld: number,
  ...
  // Random activities during hold period
  ...
  for (let i = 0; i < numActivities; i++) {
    ...
    switch (activityType) {
      case 'stop_moved':
        description = `Stop loss adjusted (trailing)`
        break
      case 'note_added':
        description = `Added trading note`
        break
      case 'price_alert':
        description = `Price alert triggered`
        break
      case 'observation':
        description = `Market observation recorded`
        break
    }
...
async function addTargets(positionId: string, entryPrice: number, finalTarget: number) {
  const targets = [
    {
      position_id: positionId,
      target_number: 1,
...
async function addNote(positionId: string, content: string) {
  const { error } = await supabase
    .from('position_notes')
    .insert({
      position_id: positionId,
...
async function main() {
  ...
  // Generate 50 closed positions
  console.log('\n📊 Creating 50 closed positions...\n')
  const closedPromises = []
  for (let i = 0; i < 50; i++) {
    const ticker = randomElement(TICKERS)
    closedPromises.push(generateClosedPosition(ticker, i))
  }
  await Promise.all(closedPromises)
  
  // Generate 10 open positions
  console.log('\n📈 Creating 10 open positions...\n')
  const openPromises = []
  for (let i = 0; i < 10; i++) {
    const ticker = TICKERS[i] // Use first 10 to ensure variety
    openPromises.push(generateOpenPosition(ticker))
  }
